...
import { construirMensajesResultado, validarLicenciaChat, acumularTokens, crearSesionMovil, obtenerEstadoSesionMovil, subirImagenSesionMovil, renderHtmlMovil, } from './chat.service';
import { llamarLLMStream } from './chat.llm';
const chatService = new ChatService();
function resolverBaseUrlParaMovil(req) {
    ...
    const hostParam = req.query?.host || req.body?.host;
    ...
    const ipLocal = candidatas.find((ip) => ip.startsWith('192.168.')) ||
...
export class ChatController {
    async crearSesionMovil(req, res) {
        try {
    ...
    async verPaginaMovil(req, res) {
        try {
            const { sessionId } = req.params;
            const html = renderHtmlMovil(sessionId);
    ...
    async subirFotoMovil(req, res) {
        try {
            ...
            const ok = subirImagenSesionMovil(sessionId, imagenBase64, mimeType, nombreArchivo);
            if (!ok) {
    ...
    async estadoSesionMovil(req, res) {
        try {
            const { sessionId } = req.params;
            const estado = obtenerEstadoSesionMovil(sessionId);
            res.json(estado);
        }
        catch (error) {
            handleApiError(error, res);
    ...
    async procesarFacturaOcr(req, res) {
        try {

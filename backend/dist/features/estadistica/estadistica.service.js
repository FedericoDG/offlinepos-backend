...
function etiquetaPeriodo(fecha) {
...
export class EstadisticaService {
     ...
     */
    async proximosVencimientos(dias) {
    ...
    async resumen() {
        ...
        const [pagosMes, pagosMesAnterior, suscripciones, totalComercios, totalLicencias, totalActivaciones, porVencer,] = await Promise.all([
        ...
        for (const s of suscripciones) {
            ...
            if (atraso <= 0)
                activas += 1;
            else if (atraso <= DIAS_GRACIA * MS_POR_DIA)
                enGracia += 1;
            else
                vencidas += 1;
    ...
    async ingresosPorPlan(meses) {
        ...
        for (const pago of pagos) {
            ...
            if (!plan)
                continue;
            const actual = acumulado.get(plan.id) ?? { plan_id: plan.id, codigo: plan.codigo, nombre: plan.nombre, total: 0, cantidad: 0 };
            actual.total += Number(pago.monto);
            actual.cantidad += 1;

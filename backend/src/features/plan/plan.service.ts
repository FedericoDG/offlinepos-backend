...
import { ActualizarPlanDTO, CrearPlanDTO, PlanDTO } from './plan.dtos';
...
export class PlanService {
  ...
  async crear(data: CrearPlanDTO): Promise<PlanDTO> {
    const existente = await this.db.plan.findUnique({ where: { codigo: data.codigo } });
    if (existente) {
      const error: any = new Error(`Ya existe un plan con el código ${data.codigo}`);
  ...
  async getById(id: string): Promise<PlanDTO> {
    ...
    if (!plan) {
      const error: any = new Error('Plan no encontrado');
  ...
  async actualizar(id: string, data: ActualizarPlanDTO): Promise<PlanDTO> {
   ...
   */
  async desactivar(id: string): Promise<PlanDTO> {
    await this.getById(id);
  ...
  async eliminar(id: string): Promise<void> {
    ...
    if (plan.suscripciones_activas > 0) {
      const error: any = new Error(
        'El plan tiene suscripciones asociadas y no se puede eliminar. Desactivalo en su lugar.'
      );
      error.statusCode = 409;
      throw error;
    }
  ...
  private serializar(plan: any): PlanDTO {
    return {
      id: plan.id,
      codigo: plan.codigo,
      nombre: plan.nombre,

 ...
 */
function coherenciaVersionInstalador(nombreArchivo, versionDeclarada) {
    ...
    if (!m)
        return; // Nombre atípico: no hay contra qué comparar, se permite.
    if (m[1] !== versionDeclarada) {
        throw httpError(`El instalador "${nombreArchivo}" parece ser de la versión ${m[1]} pero declaraste la ${versionDeclarada}. Subí el build correspondiente o corregí la versión: publicar así dejaría a las cajas ofreciéndose una actualización que nunca converge.`, 400);
    }
}
...
export function dirUpdates() {
    return RAIZ_UPDATES;
}
export function leerLatest() {
    try {
...
function archivosDe(dirVersion, version) {
    try {
        return fs
...
export function listarVersiones() {
    ...
    return carpetas
        ...
        .sort((a, b) => {
        if (a.esVigente !== b.esVigente)
            return a.esVigente ? -1 : 1;
 ...
 */
export function eliminarVersion(versionCruda) {
 ...
 */
export function publicarVersion(subida) {

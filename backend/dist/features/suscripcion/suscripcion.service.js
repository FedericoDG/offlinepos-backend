...
import { AJUSTE_VACIO, sincronizarLicenciasConPlan, } from '../licencia/licencia.provision';
...
export class SuscripcionService {
    ...
    async crear(data) {
        ...
        if (!comercio)
            throw httpError('Comercio no encontrado', 404);
        if (!plan)
        ...
        const precioLista = data.ciclo === 'ANUAL'
    ...
    async getById(id) {
        ...
        if (!suscripcion)
            throw httpError('Suscripción no encontrada', 404);
    ...
    async actualizar(id, data) {
        ...
        const planNuevo = data.plan_id && data.plan_id !== actual.plan_id
    ...
    estadoEfectivo(estado, vence) {
        ...
        if (ahora <= vencimiento + DIAS_GRACIA * MS_POR_DIA)
            return EstadoSuscripcion.EN_GRACIA;
        return EstadoSuscripcion.VENCIDA;
    }
    serializar(s, ajuste = AJUSTE_VACIO) {
        ...
        return {
            ...
            plan: s.plan
                ...
                : null,
            pagos: (s.pagos ?? []).map((p) => ({ ...p, monto: Number(p.monto) })),

...
export class PagoService {
    ...
    async getAll(filtro) {
        const where = {};
        if (filtro.suscripcion_id)
            where.suscripcion_id = filtro.suscripcion_id;
        if (filtro.metodo)
            where.metodo = filtro.metodo;
        ...
        if (filtro.comercio_id)
            porComercio.comercio_id = filtro.comercio_id;
        if (filtro.q)
            porComercio.comercio = { nombre: { contains: filtro.q, mode: 'insensitive' } };
        if (Object.keys(porComercio).length > 0)
            where.suscripcion = porComercio;
    ...
    async getById(id) {
        ...
        if (!pago)
            throw httpError('Pago no encontrado', 404);
        return this.serializar(pago);
    }
    async eliminar(id) {
